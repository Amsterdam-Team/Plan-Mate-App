...
class UpdateStateUiControllerTest {
    ...
    @Test
    fun `should execute edit state successfully when all inputs are valid`() = runTest {
        //Given
        val projectID = "db373589-b656-4e68-a7c0-2ccc705ca169"
        val oldState = "Done"
        val newState = "Completed"
        every { consoleIO.println(capture(slot)) } just Runs
        coEvery { usecase.updateState(projectID, oldState, newState) } returns true
        //When
    ...
    @Test
    fun `should show error message when updateStateUseCase throws InvalidStateName`() = runTest {
        //Given
        val projectID = "db373589-b656-4e68-a7c0-2ccc705ca169"
        val oldState = " "
        val newState = "Done"
        ...
        coEvery  { usecase.updateState(projectID, oldState, newState) } throws InvalidStateNameException
        //When
        uiController.execute()
        //Then
    ...
    fun `should show invalid Id format message when updateStateUseCase throws InvalidProjectIDException`(invalidID: String)= runTest {
        //Given
        val oldState = "Done"
        val newState = "Finished"
        every { consoleIO.println(capture(slot)) } just Runs
        every { consoleIO.readFromUser() } returnsMany listOf(invalidID, oldState, newState)
        coEvery  { usecase.updateState(invalidID, oldState, newState) } throws InvalidProjectIDException
        //When
        uiController.execute()
        //Then
        assertThat(slot.containsAll(listOf("Enter New State :)","Enter Current State :)","Enter project ID :)")))
    ...
    @Test
    fun `should show no change message when updateStateUseCase throws SameStateNameException`() = runTest{
        //Given
        val projectID = "db373589-b656-4e68-a7c0-2ccc705ca169"
        val oldState = "Done"
        val newState = "Done"
        every { consoleIO.println(capture(slot)) } just Runs
        every { consoleIO.readFromUser() } returnsMany listOf(projectID, oldState, newState)
        coEvery  { usecase.updateState(projectID, oldState, newState) } throws SameStateNameException
        //When
        uiController.execute()
        //Then
        assertThat(slot.containsAll(listOf("Enter New State :)","Enter Current State :)","Enter project ID :)")))
