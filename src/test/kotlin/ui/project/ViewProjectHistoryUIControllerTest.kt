...
class ViewProjectHistoryUIControllerTest {
    ...
    @Test
    fun `should call read from user function`(){
        //When
        controller.execute()
        //Then
    ...
    @Test
    fun `should call viewProjectHistoryUseCase when user enter the input`() {
        // Given
        ...
        // Then
        verify(exactly = 1) { useCase.execute(selectedProjectId) }
    }
    ...
    fun `should print logs for selected project`() {
        ...
        val selectedProjectId = PROJECT_1.id.toString()
        every { consoleIO.readFromUser() } returns selectedProjectId
        every { useCase.execute(selectedProjectId)} returns LOGS_FOR_PROJECT_1
        // When
        controller.execute()
